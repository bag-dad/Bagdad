<?php
/**
 * Main class for Bagdad, holds everything.
 *
 * @package BagdadCore
 */
class CBagdad implements ISingleton {
...
// include the site specific config.php and create a ref to $ba to be used by config.php
$ba = &$this;
    require(BAGDAD_SITE_PATH.'/config.php');
 ...
 * @return CBagdad The instance of this class.
 */
public static function Instance() {
if(self::$instance == null) {
self::$instance = new CBagdad();
}
 ...
 */
  public function ThemeEngineRender() {
    ...
    $themePath = BAGDAD_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = BAGDAD_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl	= $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $ba->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $ba
    ...
    $ba = &$this;
    // First the default Bagdad themes/functions.php
    include(BAGDAD_INSTALL_PATH . '/themes/functions.php');
...
/**
 * Draw HTML for a menu defined in $ba->config['menus'].
 *
